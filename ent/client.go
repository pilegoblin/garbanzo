...
import (
	...
	"github.com/pilegoblin/garbanzo/ent/migrate"
...
type Client struct {
	...
	// Bean is the client for interacting with the Bean builders.
	Bean *BeanClient
...
// BeanClient is a client for the Bean schema.
type BeanClient struct {
	config
}

// NewBeanClient returns a client for the Bean from the given config.
func NewBeanClient(c config) *BeanClient {
	return &BeanClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `bean.Hooks(f(g(h())))`.
func (c *BeanClient) Use(hooks ...Hook) {
	c.hooks.Bean = append(c.hooks.Bean, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `bean.Intercept(f(g(h())))`.
func (c *BeanClient) Intercept(interceptors ...Interceptor) {
	c.inters.Bean = append(c.inters.Bean, interceptors...)
}

// Create returns a builder for creating a Bean entity.
func (c *BeanClient) Create() *BeanCreate {
	mutation := newBeanMutation(c.config, OpCreate)
	return &BeanCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Bean entities.
func (c *BeanClient) CreateBulk(builders ...*BeanCreate) *BeanCreateBulk {
	return &BeanCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *BeanClient) MapCreateBulk(slice any, setFunc func(*BeanCreate, int)) *BeanCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &BeanCreateBulk{err: fmt.Errorf("calling to BeanClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*BeanCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
// Update returns an update builder for Bean.
func (c *BeanClient) Update() *BeanUpdate {
	mutation := newBeanMutation(c.config, OpUpdate)
	return &BeanUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *BeanClient) UpdateOne(b *Bean) *BeanUpdateOne {
	mutation := newBeanMutation(c.config, OpUpdateOne, withBean(b))
	return &BeanUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *BeanClient) UpdateOneID(id int) *BeanUpdateOne {
	mutation := newBeanMutation(c.config, OpUpdateOne, withBeanID(id))
	return &BeanUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Bean.
func (c *BeanClient) Delete() *BeanDelete {
	mutation := newBeanMutation(c.config, OpDelete)
	return &BeanDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *BeanClient) DeleteOne(b *Bean) *BeanDeleteOne {
	return c.DeleteOneID(b.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *BeanClient) DeleteOneID(id int) *BeanDeleteOne {
	builder := c.Delete().Where(bean.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &BeanDeleteOne{builder}
}

// Query returns a query builder for Bean.
func (c *BeanClient) Query() *BeanQuery {
	return &BeanQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeBean},
		inters: c.Interceptors(),
	}
}

// Get returns a Bean entity by its id.
func (c *BeanClient) Get(ctx context.Context, id int) (*Bean, error) {
	return c.Query().Where(bean.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *BeanClient) GetX(ctx context.Context, id int) *Bean {
...
// QueryPosts queries the posts edge of a Bean.
func (c *BeanClient) QueryPosts(b *Bean) *PostQuery {
...
func (c *BeanClient) Interceptors() []Interceptor {
	return c.inters.Bean
}

func (c *BeanClient) mutate(ctx context.Context, m *BeanMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&BeanCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&BeanUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&BeanUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&BeanDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Bean mutation op: %q", m.Op())
	}
}
...
// UpdateOneID returns an update builder for the given id.
func (c *PodClient) UpdateOneID(id int) *PodUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *PodClient) DeleteOneID(id int) *PodDeleteOne {
...
func (c *PodClient) Get(ctx context.Context, id int) (*Pod, error) {
	return c.Query().Where(pod.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *PodClient) GetX(ctx context.Context, id int) *Pod {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryOwner queries the owner edge of a Pod.
func (c *PodClient) QueryOwner(po *Pod) *UserQuery {
...
// QueryUsers queries the users edge of a Pod.
func (c *PodClient) QueryUsers(po *Pod) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := po.ID
		step := sqlgraph.NewStep(
...
// QueryBean queries the bean edge of a Post.
func (c *PostClient) QueryBean(po *Post) *BeanQuery {
	query := (&BeanClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := po.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(post.Table, post.FieldID, id),
			sqlgraph.To(bean.Table, bean.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, post.BeanTable, post.BeanColumn),
		)
		fromV = sqlgraph.Neighbors(po.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryOwnedPods queries the owned_pods edge of a User.
func (c *UserClient) QueryOwnedPods(u *User) *PodQuery {
	query := (&PodClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(pod.Table, pod.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.OwnedPodsTable, user.OwnedPodsColumn),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
...
// QueryJoinedPods queries the joined_pods edge of a User.
func (c *UserClient) QueryJoinedPods(u *User) *PodQuery {
	query := (&PodClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(pod.Table, pod.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, user.JoinedPodsTable, user.JoinedPodsPrimaryKey...),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Bean, Pod, Post, User []ent.Hook
	}
	inters struct {
		Bean, Pod, Post, User []ent.Interceptor
	}
)
