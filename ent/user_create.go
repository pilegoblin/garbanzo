...
func (uc *UserCreate) AddOwnedPodIDs(ids ...int) *UserCreate {
	uc.mutation.AddOwnedPodIDs(ids...)
	return uc
}

// AddOwnedPods adds the "owned_pods" edges to the Pod entity.
func (uc *UserCreate) AddOwnedPods(p ...*Pod) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddOwnedPodIDs(ids...)
}

// AddJoinedPodIDs adds the "joined_pods" edge to the Pod entity by IDs.
func (uc *UserCreate) AddJoinedPodIDs(ids ...int) *UserCreate {
	uc.mutation.AddJoinedPodIDs(ids...)
	return uc
}

// AddJoinedPods adds the "joined_pods" edges to the Pod entity.
func (uc *UserCreate) AddJoinedPods(p ...*Pod) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddJoinedPodIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.OwnedPodsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.JoinedPodsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
