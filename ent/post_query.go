...
// QueryBean chains the current query on the "bean" edge.
func (pq *PostQuery) QueryBean() *BeanQuery {
...
// WithBean tells the query-builder to eager-load the nodes that are connected to
// the "bean" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithBean(opts ...func(*BeanQuery)) *PostQuery {
	query := (&BeanClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if pq.withUser != nil || pq.withBean != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withBean; query != nil {
		if err := pq.loadBean(ctx, query, nodes, nil,
			func(n *Post, e *Bean) { n.Edges.Bean = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadBean(ctx context.Context, query *BeanQuery, nodes []*Post, init func(*Post), assign func(*Post, *Bean)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Post)
	for i := range nodes {
		if nodes[i].bean_posts == nil {
			continue
		}
		fk := *nodes[i].bean_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "bean_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
